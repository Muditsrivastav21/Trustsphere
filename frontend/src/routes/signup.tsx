...
function SignupPage() {
  ...
  const [formData, setFormData] = useState({
    name: "", email: "", phone: "", dob: "", id_number: "", password: ""
  });
          ...
          <div className="bg-[var(--color-navy)]/60 backdrop-blur-xl border border-white/5 shadow-2xl rounded-2xl p-8 sm:p-10">
            ...
            {loading ? (
              <div className="py-8 flex flex-col items-center justify-center space-y-8 animate-fade-in">
                ...
                <div className="w-full space-y-4 bg-black/20 backdrop-blur-sm p-6 rounded-xl border border-white/5 shadow-inner">
                  <div className={`flex items-center gap-4 transition-all duration-500 ${step >= 0 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    ...
                    <span className={`text-sm font-medium ${step > 0 ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-primary)]'}`}>Evaluating behavioral biometrics</span>
                  </div>
                  <div className={`flex items-center gap-4 transition-all duration-500 ${step >= 1 ? 'opacity-100 translate-y-0' : 'opacity-20 translate-y-4'}`}>
                    ...
                    <span className={`text-sm font-medium ${step > 1 ? 'text-[var(--color-text-secondary)]' : step === 1 ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-muted)]'}`}>Checking KYC / AML signals</span>
                  </div>
              ...
              <form onSubmit={submit} className="space-y-4 animate-fade-in" onKeyDown={onKeyDown} onKeyUp={onKeyUp}>
                ...
                <div className="group">
                  <label className="label-caps text-[var(--color-text-secondary)] block mb-1.5">National ID / SSN</label>
                  <input required name="id_number" value={formData.id_number} onChange={handleChange} className="w-full px-4 py-3 bg-black/20 border border-white/5 rounded-xl text-sm placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-bob-orange)] focus:ring-1 focus:ring-[var(--color-bob-orange)]/50 transition-all text-white" />
                </div>
